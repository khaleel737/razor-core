...
public final class BatchAddSymbolsCommand implements BinaryDataCommand {
    ...
    private final IntObjectHashMap<SymbolSpecification> symbols;

    public BatchAddSymbolsCommand(final SymbolSpecification symbol) {
        symbols = IntObjectHashMap.newWithKeysValues(symbol.symbolId, symbol);
    }

    public BatchAddSymbolsCommand(final Collection<SymbolSpecification> collection) {
    ...
    public BatchAddSymbolsCommand(final BytesIn bytes) {
        symbols = SerializationUtils.readIntHashMap(bytes, SymbolSpecification::new);
    }
