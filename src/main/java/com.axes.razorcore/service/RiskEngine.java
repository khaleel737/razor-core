...
import com.axes.razorcore.utils.*;
...
public final class RiskEngine implements WriteBytesMarshallable {
    ...
    public RiskEngine(final int shardId,
                      ...
                      final ApplicationConfig exchangeConfiguration) {
    ...
    public boolean preProcessCommand(final long seq, final OrderCommand cmd) {
        switch (cmd.commandType) {
            ...
            case ADD_USER:
                if (uidForThisHandler(cmd.uuid)) {
                    cmd.resultCode = userProfileService.addEmptyUserProfile(cmd.uuid)
                            ? CommandResultCode.SUCCESS
                            : CommandResultCode.USER_MANAGEMENT_USER_ALREADY_EXISTS;
                }
                return false;
            ...
            case BALANCE_ADJUSTMENT:
                if (uidForThisHandler(cmd.uuid)) {
                    cmd.resultCode = adjustBalance(
                            cmd.uuid, cmd.symbol, cmd.price, cmd.orderId, BalanceAdjustedType.of(cmd.orderType.getCode()));
                }
                return false;

            case SUSPEND_USER:
                if (uidForThisHandler(cmd.uuid)) {
                    cmd.resultCode = userProfileService.suspendUserProfile(cmd.uuid);
                }
                return false;
            case RESUME_USER:
                if (uidForThisHandler(cmd.uuid)) {
                    cmd.resultCode = userProfileService.resumeUserProfile(cmd.uuid);
                }
                return false;
            ...
            case PERSIST_STATE_RISK:
                final boolean isSuccess = serializationProcessor.storeData(
                        ...
                        shardId,
                        this);
                UnsafeUtils.setResultVolatile(cmd, isSuccess, CommandResultCode.SUCCESS, CommandResultCode.STATE_PERSIST_MATCHING_ENGINE_FAILED);
                return false;
        }
        return false;
    }
    ...
    private CommandResultCode adjustBalance(long uid, int currency, long amountDiff, long fundingTransactionId, BalanceAdjustedType adjustmentType) {
        ...
        if (res == CommandResultCode.SUCCESS) {
            switch (adjustmentType) {
                case ADJUSTED: // adjust total adjustments amount
                    adjustments.addToValue(currency, -amountDiff);
                    break;

                case SUSPENDED: // adjust total suspends amount
            ...
            ((BatchAddAccountsCommand) message).getUsers().forEachKeyValue((uid, accounts) -> {
                if (userProfileService.addEmptyUserProfile(uid)) {
                    accounts.forEachKeyValue((cur, bal) ->
                            adjustBalance(uid, cur, bal, 1_000_000_000 + cur, BalanceAdjustedType.ADJUSTED));
                } else {
    ...
    private CommandResultCode placeOrderRiskCheck(final OrderCommand cmd) {
        ...
        final UserProfile userProfile = userProfileService.getUserProfile(cmd.uuid);
        if (userProfile == null) {
            cmd.resultCode = CommandResultCode.AUTH_INVALID_USER;
            log.warn("User profile {} not found", cmd.uuid);
            return CommandResultCode.AUTH_INVALID_USER;
        }

        final SymbolSpecification spec = symbolSpecificationProvider.getSymbolSpecification(cmd.symbol);
        if (spec == null) {
        ...
        if (resultCode != CommandResultCode.VALID_FOR_MATCHING_ENGINE) {
            log.warn("{} risk result={} uid={}: Can not place {}", cmd.orderId, resultCode, userProfile.uuid, cmd);
    ...
    private CommandResultCode placeOrder(final OrderCommand cmd,
                                         ...
                                         final SymbolSpecification spec) {


        if (spec.type == SymbolType.CURRENCY_EXCHANGE_PAIRS) {

            return placeExchangeOrder(cmd, userProfile, spec);

        } else if (spec.type == SymbolType.FUTURES_CONTRACTS) {

            if (!cfgMarginTradingEnabled) {
                return CommandResultCode.RISK_MARGIN_TRADING_DISABLED;
            }

            SymbolPosition position = userProfile.userPositions.get(spec.symbolId); // TODO getIfAbsentPut?
            if (position == null) {
                position = objectsPool.get(ObjectsPool.SYMBOL_POSITION_RECORD, SymbolPosition::new);
                position.initialize(userProfile.uuid, spec.symbolId, spec.quoteCurrency);
                userProfile.userPositions.put(spec.symbolId, position);
            }
    ...
    private CommandResultCode placeExchangeOrder(final OrderCommand cmd,
        ...
        if (cfgMarginTradingEnabled) {
            for (final SymbolPosition position : userProfile.userPositions) {
                if (position.currency == currency) {
                    final int recSymbol = position.symbol;
                    final SymbolSpecification spec2 = symbolSpecificationProvider.getSymbolSpecification(recSymbol);
        ...
        if (logDebug) {
            log.debug("R1 uid={} : orderHoldAmount={} vs serProfile.accounts.get({})={} + freeFuturesMargin={}",
                    userProfile.uuid, orderHoldAmount, currency, userProfile.accounts.get(currency), freeFuturesMargin);
        }
...
//            log.warn("orderAmount={} > userProfile.accounts.get({})={}", orderAmount, currency, userProfile.accounts.get(currency));
    ...
    private boolean canPlaceMarginOrder(final OrderCommand cmd,
        ...
        long freeMargin = 0L;
        for (final SymbolPosition positionRecord : userProfile.userPositions) {
...
//                newRequiredMarginForSymbol, position.currency, userProfile.accounts.get(position.currency), freeMargin);
    ...
    public boolean handlerRiskRelease(final long seq, final OrderCommand cmd) {
        ...
        MatchTradeEventHandler mte = cmd.matchTradeEventHandler;

        // skip events processing if no events (or if contains BINARY EVENT)
        if (marketData == null && (mte == null || mte.matchEventType == MatchEventType.BINARY_EVENT)) {
            return false;
        }

        final SymbolSpecification spec = symbolSpecificationProvider.getSymbolSpecification(symbol);
        if (spec == null) {
        ...
        if (mte != null && mte.matchEventType != MatchEventType.BINARY_EVENT) {
            // at least one event to process, resolving primary/taker user profile
            // TODO processing order is reversed
            if (spec.type == SymbolType.CURRENCY_EXCHANGE_PAIRS) {

                final UserProfile takerUp = uidForThisHandler(cmd.uuid)
                        ? userProfileService.getUserProfileOrAddSuspended(cmd.uuid)
                        : null;

                // REJECT always comes first; REDUCE is always single event
                if (mte.matchEventType == MatchEventType.REDUCE || mte.matchEventType == MatchEventType.REJECT) {
                    if (takerUp != null) {
                        handleMatcherRejectReduceEventExchange(cmd, mte, spec, takerSell, takerUp);
                    }
                    mte = mte.matchTradeNextEvent;
                }
                ...
                final UserProfile takerUp = uidForThisHandler(cmd.uuid) ? userProfileService.getUserProfileOrAddSuspended(cmd.uuid) : null;

                // for margin-mode symbols also resolve position record
                final SymbolPosition takerSpr = (takerUp != null) ? takerUp.getPositionRecordOrThrowException(symbol) : null;
                do {
    ...
    private void handleMatcherEventMargin(final MatchTradeEventHandler ev,
                                          final SymbolSpecification spec,
                                          final OrderAction takerAction,
                                          final UserProfile takerUp,
                                          final SymbolPosition takerSpr) {
        if (takerUp != null) {
            ...
            } else if (ev.matchEventType == MatchEventType.REJECT || ev.matchEventType == MatchEventType.REDUCE) {
    ...
    private void handleMatcherRejectReduceEventExchange(final OrderCommand cmd,
                                                        final MatchTradeEventHandler ev,
                                                        final SymbolSpecification spec,
        ...
        if (takerSell) {
            ...
            if (cmd.commandType == OrderCommandType.PLACE_ORDER && cmd.orderType == OrderType.FOK_BUDGET) {
    ...
    private void handleMatcherEventsExchangeSell(MatchTradeEventHandler ev,
                                                 final SymbolSpecification spec,
                                                 final UserProfile taker) {
        ...
        while (ev != null) {
            assert ev.matchEventType == MatchEventType.TRADE;
            ...
            if (uidForThisHandler(ev.matchedPositionsId)) {
                final long size = ev.size;
                final UserProfile maker = userProfileService.getUserProfileOrAddSuspended(ev.matchedPositionsId);
    ...
    private void handleMatcherEventsExchangeBuy(MatchTradeEventHandler ev,
                                                final SymbolSpecification spec,
                                                final UserProfile taker,
