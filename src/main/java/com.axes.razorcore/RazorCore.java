...
import com.axes.razorcore.exception.DisruptorException;
...
import com.axes.razorcore.service.GroupingProcessor;
import com.axes.razorcore.service.MatchingEngineRouter;
import com.axes.razorcore.service.ResultsHandler;
import com.axes.razorcore.service.RiskEngine;
import com.axes.razorcore.utils.TwoStepMasterProcessor;
import com.axes.razorcore.utils.TwoStepSlaveProcessor;
...
public final class RazorCore {
    ...
    private final ApplicationConfig exchangeConfiguration;
     ...
     */
    @Builder
    public RazorCore(final ObjLongConsumer<OrderCommand> resultsConsumer,
        ...
        final RazorCoreWaitStrategy coreWaitStrategy = perfCfg.getWaitStrategy();
        ...
        this.disruptor = new Disruptor<>(
                ...
                coreWaitStrategy.getDisruptorWaitStrategy().get());

        this.ringBuffer = disruptor.getRingBuffer();

        this.api = new RazorCoreApi(ringBuffer, perfCfg.getBinaryCommandsLz4CompressorFactory().get());
        ...
        final SharedPooling sharedPool = new SharedPooling(poolInitialSize * 4, poolInitialSize, chainLength);

        // creating and attaching exceptions handler
        final DisruptorException<OrderCommand> exceptionHandler = new DisruptorException<>("main", (ex, seq) -> {
        ...
        final EventHandler<OrderCommand>[] matchingEngineHandlers = matchingEngineFutures.values().stream()
                ...
                .toArray(RazorCore::newEventHandlersArray);
        ...
        IntStream.range(0, riskEnginesNum).forEach(i -> procR1.get(i).setTwoStepSlaveProcessor(procR2.get(i)));
    ...
    /**
     * Provides RazorCoreApi instance.
     *
     * @return RazorCoreApi instance (always same object)
     */
    public RazorCoreApi getApi() {
        return api;
    }

    private static final EventTranslator<OrderCommand> SHUTDOWN_SIGNAL_TRANSLATOR = (cmd, seq) -> {
        cmd.commandType = OrderCommandType.SHUTDOWN_SIGNAL;
