...
import com.axes.razorcore.service.SymbolSpecificationProvider;
import com.axes.razorcore.utils.CoreArithmeticUtils;
...
public final class TotalCurrencyBalanceReportQuery implements ReportQuery<TotalCurrencyBalanceReportResult> {
    ...
    public Optional<TotalCurrencyBalanceReportResult> process(final MatchingEngineRouter matchingEngine) {
        ...
        matchingEngine.getOrderBooks().stream()
                .filter(ob -> ob.getSymbolSpec().type == SymbolType.CURRENCY_EXCHANGE_PAIRS)
                .forEach(ob -> {
                    final SymbolSpecification spec = ob.getSymbolSpec();
    ...
    public Optional<TotalCurrencyBalanceReportResult> process(final RiskEngine riskEngine) {
        ...
        riskEngine.getUserProfileService().getUserProfiles().forEach(userProfile -> {
            ...
            userProfile.userPositions.forEachKeyValue((symbolId, positionRecord) -> {
                final SymbolSpecification spec = symbolSpecificationProvider.getSymbolSpecification(symbolId);
                ...
                if (positionRecord.orderDirection == OrderDirection.LONG) {
                    symbolOpenInterestLong.addToValue(symbolId, positionRecord.openVolume);
                } else if (positionRecord.orderDirection == OrderDirection.SHORT) {
