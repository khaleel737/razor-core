...
import com.axes.razorcore.config.DiskSerializationProcessorConfiguration;
...
import com.axes.razorcore.journaling.JournalDescriptor;
...
//    final SingleWriterRecorder hdrRecorderLz4 = new SingleWriterRecorder(Integer.MAX_VALUE, 2);
    ...
    public DiskSerializationProcessor(ApplicationConfig exchangeConfig,
...
//            bytes.release();
    ...
    // single threaded
    @Override
    public void writeToJournal(OrderCommand command, long dSeq, boolean eob) throws IOException {
...
//        log.debug("Writing {}", cmd);
        ...
        if (commandType == OrderCommandType.SHUTDOWN_SIGNAL) {
            flushBufferSync(false, command.timestamp);
            log.debug("Shutdown signal received, flushed to disk");
            return;
        }

        if (!commandType.isMutate()) {
            // skip queries
            return;
        }

        if (channel == null) {
            startNewFile(command.timestamp);
        }

        final ByteBuffer buffer = journalWriteBuffer;

        // mandatory fields
        buffer.put(commandType.getCode()); // 1 byte
        buffer.putLong(baseSeq + dSeq); // 8 bytes - can be compressed as delta
        buffer.putLong(command.timestamp); // 8 bytes - can be compressed as delta
        buffer.putInt(command.serviceFlags); // 4 bytes - can be compressed as dictionary
        buffer.putLong(command.eventsGroup); // 8 bytes - can be compressed as delta

        if (debug)
            log.debug("LOG {} eventsGroup={} serviceFlags={}", String.format("seq=%d t=%d command=%X (%s) ", baseSeq + dSeq, command.timestamp, commandType.getCode(), commandType), command.eventsGroup, command.serviceFlags);

        if (commandType == OrderCommandType.MOVE_ORDER) {

            buffer.putLong(command.uuid); // 8 bytes can be compressed as dictionary
            buffer.putInt(command.symbol); // 4 bytes can be compressed as dictionary
            buffer.putLong(command.orderId); // 8 bytes - can be compressed as delta
            buffer.putLong(command.price); // 8 bytes - can be compressed as delta

            if (debug) log.debug("move order seq={} t={} orderId={} symbol={} uid={} price={}", baseSeq + dSeq, command.timestamp, command.orderId, command.symbol, command.uuid, command.price);
            ...
            if (debug) log.debug("cancel order seq={} t={} orderId={} symbol={} uid={}", baseSeq + dSeq, command.timestamp, command.orderId, command.symbol, command.uuid);
            ...
            buffer.putInt(command.symbol); // 4 bytes can be compressed as dictionary
            buffer.putLong(command.orderId); // 8 bytes - can be compressed as delta
            buffer.putLong(command.size); // 8 bytes - can be compressed as low value

            if (debug) log.debug("reduce order seq={} t={} orderId={} symbol={} uid={} size={}", baseSeq + dSeq, command.timestamp, command.orderId, command.symbol, command.uuid, command.size);
            ...
            buffer.putLong(command.orderId); // 8 bytes - can be compressed as delta
            buffer.putLong(command.price); // 8 bytes - can be compressed as delta
            buffer.putLong(command.reserveBidPrice); // 8 bytes - can be compressed (diff to price or 0)
            buffer.putLong(command.size); // 8 bytes - can be compressed
            buffer.putInt(command.userCookies); // 4 bytes can be log-compressed

            final int actionAndType = (command.orderType.getCode() << 1) | command.action.getCode();
            ...
            if (debug) log.debug("place order seq={} t={} orderId={} symbol={} uid={} price={} reserveBidPrice={} size={} userCookie={} {}/{} actionAndType={}",
                    baseSeq + dSeq, command.timestamp, command.orderId, command.symbol, command.uuid, command.price, command.reserveBidPrice, command.size, command.userCookies, command.action, command.orderType, actionAndType1);

        } else if (commandType == OrderCommandType.BALANCE_ADJUSTMENT) {

            buffer.putLong(command.uuid); // 8 bytes can be compressed as dictionary
            buffer.putInt(command.symbol); // 4 bytes can be compressed as dictionary (currency)
            buffer.putLong(command.orderId); // 8 bytes can be compressed as delta (transaction)
            buffer.putLong(command.price); // 8 bytes - can be compressed as low value (amount)
            buffer.put(command.orderType.getCode()); // 1 byte (adjustment or suspend)

        } else if (commandType == OrderCommandType.ADD_USER ||
                commandType == OrderCommandType.SUSPEND_USER ||
                commandType == OrderCommandType.RESUME_USER) {

            buffer.putLong(command.uuid); // 8 bytes can be compressed as delta

        } else if (commandType == OrderCommandType.BINARY_DATA_COMMAND) {

//            if (debug) log.debug("LOG BINARY_DATA_COMMAND {}", String.format("seq=%d f=%d word0=%X word1=%X word2=%X word3=%X word4=%X",
//                    dSeq + baseSeq, (byte) command.symbol, command.orderId, command.price, command.reserveBidPrice, command.size, command.uuid));

            buffer.put((byte) command.symbol); // 1 byte (0 or -1)
            buffer.putLong(command.orderId); // 8 bytes word0
            buffer.putLong(command.price); // 8 bytes word1
            buffer.putLong(command.reserveBidPrice); // 8 bytes word2
            buffer.putLong(command.size); // 8 bytes word3
            buffer.putLong(command.uuid); // 8 bytes word4

//        } else if (commandType == OrderCommandType.PERSIST_STATE_MATCHING ||
//                commandType == OrderCommandType.PERSIST_STATE_RISK) {
//            buffer.putLong(command.orderId); // 8 bytes
        }

        if (commandType == OrderCommandType.PERSIST_STATE_RISK) {

            // register snapshot change
            registerNextSnapshot(command.orderId, baseSeq + dSeq, command.timestamp);

            // start new file
            baseSnapshotId = command.orderId;
            filesCounter = 0;

            flushBufferSync(true, command.timestamp);

        } else if (commandType == OrderCommandType.RESET) {
    ...
    @Override
    public long replayJournalFull(InitialStateConfiguration initialCfg, RazorCoreApi api) {
...
//            boolean debug = insideCompressedBlock;
            ...
            final byte command = jr.readByte();

            if (debug) log.debug("COMPR STEP lastSeq={} ", lastSeq);

            if (command == OrderCommandType.RESERVED_COMPRESSED.getCode()) {
...
//                log.debug("Decoding block {}", origSize);
                ...
                final OrderCommandType commandType = OrderCommandType.fromCode(command);

                if (seq != lastSeq.value + 1) {
                    log.warn("Sequence gap {}->{} ({})", lastSeq, seq, seq - lastSeq.value);
//                    log.debug("timestampNs={} eventsGroup={} serviceFlags={} commandType={}", timestampNs, eventsGroup, serviceFlags, commandType);
                }

                lastSeq.value = seq;

//                log.debug("command seq={} {}", lastSeq, commandType);

                if (debug) log.debug("eventsGroup={} serviceFlags={} commandType={}", eventsGroup, serviceFlags, commandType);
                ...
                if (commandType == OrderCommandType.MOVE_ORDER) {
                    ...
                    final BalanceAdjustedType adjustmentType = BalanceAdjustedType.of(jr.readByte()); // 1 byte (adjustment or suspend)
                ...
                } else if (commandType == OrderCommandType.RESUME_USER) {
                    ...
                    log.debug("eventsGroup={} serviceFlags={} commandType={}", eventsGroup, serviceFlags, commandType);
